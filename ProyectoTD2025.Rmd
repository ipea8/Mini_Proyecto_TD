...
Como podemos observar, los cinco productos más vendidos dentro de la categoría de frutas y verduras son: naranja	con 82.062Kg, patata con	74.800Kg, mandarina	con 31.186Kg, cebolla con	22.000Kg y pepino	con 19.624Kg.
\newpage
...
# Los cinco productos de pescado más vendidos son bacaladilla con 7,372Kg, rodaballo con	3.672Kg, sepia lonja con 3.584Kg, dorada con	2.934Kg y sepia fresca con	2.642Kg.
```

Como podemos observar, los cinco productos de la categoría pescado más vendidos por kilos son:bacaladilla con 7,4Kg, rodaballo con	3.672Kg, sepia lonja con 3.584Kg, dorada con	2.934Kg y sepia fresca con	2.642Kg.
\newpage
...
En la tabla podemos ver el resultado de la procedencia de los tickets, siendo valencia la ciudad más frecuente con 130 tickets, seguido de alboraia con 50 tickets y después burjassot con 26 tickets.
\newpage
...
Finalmente, se construye un gráfico de barras que muestra claramente cuántos tickets se han generado en cada día. Esto permite visualizar qué días son más activos y cuáles menos, ayudando a responder la pregunta de negocio:
¿Qué día sería el más adecuado para cerrar?
...
ggplot(dia_semana, aes(x = semana, y = Freq, fill = semana)) +
  ...
  labs(title = "Tickets recogidos cada día de las semana",
    x = "Día de la semana",
    y = "Número de tickets") + theme_minimal()
```

Como se observa en la Tabla, si se tuviese que cerrar un día entre semana seria el jueves.
\newpage
## Preguntas planteadas por nosotros

**7.  ¿Cuál es la media de ventas para cada día de la semana?**

Este código tiene como objetivo procesar y analizar tickets de compra en formato PDF. Primero, extrae la fecha de cada ticket y la convierte en un formato de fecha legible por R. Luego, identifica dónde comienzan los productos dentro del ticket para separar esa sección del resto del contenido. A partir de ahí, filtra solo las líneas que corresponden a productos realmente comprados.

Después, cuenta cuántos productos hay por ticket y asocia esta cantidad con su respectiva fecha. También se añade el día de la semana en que se hizo la compra. Finalmente, se agrupan los tickets por día de la semana y se calcula la media de productos comprados por día, ordenando los días de forma cronológica para facilitar el análisis de patrones de consumo.
...
Finalmente, en esta tabla podemos ver el resultado: la media de productos los lunes es de 34, la de los martes es de 30, la de los miércoles es de 32, la de los jueves es de 27, la de los viernes es de 32, la de los sábados es de 36 y la de los domingos es de 30. Podemos observar que, de normal, el dia que menos productos se venden es el jueves.
\newpage
...
Esta pregunta analiza cómo ha cambiado el precio por kilo de los plátanos a lo largo del tiempo usando los tickets de Mercadona en PDF. Primero, se filtran las líneas de los tickets que contienen la palabra "plátano" y se extraen los nombres de los archivos donde aparecen. Luego, se obtienen las fechas asociadas a esos tickets (suponiendo que la fecha está en la línea 5) y se añaden al dataframe.

Después, se localiza la línea siguiente a la palabra "plátano", que normalmente contiene información sobre el peso y precio. De ahí se extrae el precio por kilo, convirtiéndolo en formato numérico. Con las fechas y precios ya preparados, se construye un dataframe con estas dos variables y se genera un gráfico de líneas que muestra cómo ha variado el precio de los plátanos por kilo a lo largo del tiempo. Esto permite detectar tendencias y cambios en los precios de ese producto.
...
# Verificar la estructura del dataframe 'platanos'
str(platanos)

# Extraer el precio por kg de la columna 'info'
platanos$precio_kg <- as.numeric(gsub(",", ".", str_extract(platanos$info, "\\d{1,2}[.,]?\\d{2}"))) 
# Reemplazar coma por punto

# Convertir las fechas a formato Date
platanos$fecha <- as.Date(platanos$fecha, format = "%Y-%m-%d")

# Crear un nuevo dataframe con fecha y precio por kg
df_platanos_precio <- platanos %>%
  select(fecha, precio_kg)

# Verificar las primeras filas del dataframe creado
head(df_platanos_precio)

# Graficar la evolución del precio por kilo a lo largo del tiempo
ggplot(df_platanos_precio, aes(x = fecha, y = precio_kg)) +
  geom_line() +
  labs(title = "Evolución del precio de los plátanos por kg",
       x = "Fecha", y = "Precio por kg (€)") +
  theme_minimal()
```

Como podemos ver en el gráfico, el precio por kilo va aumentando y disminuyendo a largo del año 2024, pero a inicios del año 2025 ha aumentado a gran escala.
\newpage
**9.  ¿Cuáles son los productos menos vendidos en el conjunto de tickets disponibles?**

Esta pregunta busca identificar los productos menos vendidos entre los tickets disponibles. Para ello, se filtran las líneas que representan productos (aquellas que empiezan por un número, indicando la cantidad comprada). Luego, se extraen la cantidad y el nombre del producto mediante expresiones regulares.

Después, se agrupan los datos por producto y se suma la cantidad total vendida de cada uno. Finalmente, se ordenan de menor a mayor según las unidades vendidas y se seleccionan los productos con menor frecuencia de compra. Así se obtiene un listado de los productos menos comprados en el conjunto de tickets analizados.
...
# Extraer partes específicas de cada línea (con str_match): cantidad, nombre del producto, y precio
mutate(matches = str_match(texto, "^(\\d+)\\s+(.*?)\\s+(\\d+,\\d{2})\\s+(\\d+,\\d{2})?$"),
...
# Agrupar y sumar las cantidades por producto (unidades vendidas por producto)
productos_menos_vendidos <- df_productos %>%
  group_by(producto) %>%
  summarise(unidades_vendidas = sum(cantidad), .groups = "drop") %>%
  
# Ordenar y obtener el top de productos menos vendidos
arrange(unidades_vendidas) %>% slice_head(n = 1000)

# Mostrar el resultado
productos_menos_vendidos
```

Como podemos observar en la tabla, de entre los productos menos vendidos se encuentra el agua mineral y las albondigas de bacala con tan solo 2 unidades, empatados con muchos otros productos.
\newpage
...
Como podemos observar, el producto más comprado por unidades en una sola transacción es la cola sin cafeina (8 unidades).
\newpage
...
Como podemos observar, las combinaciones más comunes son brotes tiernos maxi y queso lonchas cabra (comprados 16 veces).
\newpage
...
Como se puede comprobar en la gráfica se compran más platanos en los meses de enero, 
febrero y marzo (es decir, en invierno), por tanto, los plátanos tienen estacionalidad.
\newpage
