...
#1. Queremos extraer la hora de todos los tickets y para eso realizamos una serie de pasos:

# La hora se encuentra en todos los tickets en la línea 5. Utilizamos la función filter() para extraer la línea 5 de cada ticket.
df_horas <- df_lineas %>%
  filter(linea == 5) %>%
  mutate(
    #la función str_extract() busca una hora en formato "hh:mm" usando expresiones regulares
  ...
  #Esta función selecciona lo que nos queremos quedar que es la hora y el nombre del   archivo.
  select(archivo, hora)

#2.Queremos saber la hora a la que se suele ir más al superpermercado:

#Utilizamos la función count() (aplicandola al data frame creado antes donde hemos extraído las horas de los tickets). Esta función cuenta cuántos tickets hay por cada hora del día (de 0 a 23) 
FrecuenciaHora_supermercado<- df_horas %>% count(hora)

#3. Para poder ver los resultados de una manera más completa, realizamos una gráfico que muestre qué hora es más frecuente.
