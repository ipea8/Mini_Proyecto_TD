...
#Mostrar el resultado creando la referencia con knitr
knitr::kable(head(top_5_productos),
             caption = "5 productos más vendidos por unidades",
             booktabs = TRUE)

```

Como podemos observar en la tabla \@ref:tabla-productosvendidos, los cinco productos más vendidos por unidades son: leche desnatada con calcio (22 unidades), barra campesina(12 unidades), chapata cristal(12 unidades), garbanzo m.cocido (12 unidades) y leche desnatada sin lactosa (12 unidades).
...
# Visualizar el resultado
    knitr::kable(media_productos_dia,
                 caption = "Media de compras de productos (por unidades) cada día de la semana",
                 booktabs = TRUE)
```

Finalmente, en la tabla \@ref:tabla-medias podemos ver el resultado: la media de productos los lunes es de 34, la de los martes es de 30, la de los miércoles es de 32, la de los jueves es de 27, la de los viernes es de 32, la de los sábados es de 36 y la de los domingos es de 30. Podemos observar que, de normal, el dia que menos productos se venden es el jueves.
...
Esta pregunta analiza cómo ha cambiado el precio por kilo de las manzanas a lo largo del tiempo usando los tickets de Mercadona en PDF. Primero, se filtran las líneas de los tickets que contienen la palabra "manzana" y se extraen los nombres de los archivos donde aparecen. Luego, se obtienen las fechas asociadas a esos tickets (suponiendo que la fecha está en la línea 5) y se añaden al dataframe.

Después, se localiza la línea siguiente a la palabra "manzana", que normalmente contiene información sobre el peso y precio. De ahí se extrae el precio por kilo, convirtiéndolo en formato numérico. Con las fechas y precios ya preparados, se construye un dataframe con estas dos variables y se genera un gráfico de líneas que muestra cómo ha variado el precio de las manzanas por kilo a lo largo del tiempo. Esto permite detectar tendencias y cambios en los precios de ese producto.
...
manzanas <- df_lineas %>% filter(str_detect(tolower(texto), "manzana"))
...
archivo_manzanas <- manzanas %>% select(archivo) #filtramos los nombres
archivo_manzanas <- as.vector(archivo_manzanas$archivo) #Convertir a vector

#Con el vector de los archivos creamos un df de las fechas en las que se comparon plátanos
df_fechas_manzanas <- df_lineas %>% filter(linea == "5") %>% right_join(data.frame(archivo = archivo_manzanas), by = "archivo")

#Filtramos para quedarnos solo con las fechas en formato texto
df_fechas_manzanas <- df_fechas_manzanas %>%  mutate(fecha = str_extract(texto, "\\b\\d{2}/\\d{2}/\\d{4}\\b")) %>% select(-texto)

#Guardamos la fecha en el df de platanos
manzanas$fecha <- df_fechas_manzanas$fecha
manzanas$fecha <- as.Date(manzanas$fecha, format = "%d/%m/%Y") #Convertimos en formato fecha

#Filtramos para hallar los kilos de plátanos en cada compra
vector_manzanas <- which(str_detect(tolower(df_lineas$texto), "manzana")) #Crea un vector con las filas en las que aparecen los plátanos

#Encontramos la linea donde aparece la cantidad de plátanos (en kg) comprada
filas_kilos_manzanas <- vector_manzanas + 1

#Añadimos las líneas de los kilos al df
manzanas$info <- df_lineas %>% slice(filas_kilos_manzanas)%>%pull(texto)

# Verificar la estructura del dataframe 'platanos'
str(manzanas)

# Extraer el precio por kg de la columna 'info'
manzanas$precio_kg <- as.numeric(gsub(",", ".", str_extract(manzanas$info, "\\d{1,2}[.,]?\\d{2}"))) 
# Reemplazar coma por punto

# Convertir las fechas a formato Date
manzanas$fecha <- as.Date(manzanas$fecha, format = "%Y-%m-%d")

# Crear un nuevo dataframe con fecha y precio por kg
df_manzanas_precio <- manzanas %>%
  select(fecha, precio_kg)

# Verificar las primeras filas del dataframe creado
head(df_manzanas_precio)

# Graficar la evolución del precio por kilo a lo largo del tiempo
ggplot(df_manzanas_precio, aes(x = fecha, y = precio_kg)) +
  geom_line() +
  labs(title = "Evolución del precio de las manzanas por kg",
...
knitr::kable(head(productos_menos_vendidos, 5),
             caption = "Productos menos vendidos",
             booktabs = TRUE)
```

Como podemos observar en la tabla \@ref:tabla-menosvendidos, de entre los productos menos vendidos se encuentra el agua mineral y las albondigas de bacalao con tan solo 2 unidades, empatados con muchos otros productos.
...
Como se puede comprobar en la gráfica \@ref:figura-platanos se compran más platanos en los meses de enero, 
