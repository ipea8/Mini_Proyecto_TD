...
Queremos saber cuántos productos en promedio se compran por ticket en cada día de la semana. Esto nos permite ver si hay algún día de la semana con más productos por ticket, lo que puede ayudar a tomar decisiones sobre la operación.
...
1. Este código busca líneas en el texto que contienen una fecha en el formato dd/mm/yyyy utilizando una expresión regular. Luego, extrae estas fechas y las convierte en un formato de fecha que R pueda reconocer (Date), lo que facilita su manipulación y análisis. Finalmente, guarda el nombre del archivo del ticket y la fecha extraída, asociando así cada ticket con su fecha correspondiente para poder hacer análisis temporales posteriormente.
...
2. El código localiza la línea que marca el inicio de la lista de productos dentro de cada ticket, que generalmente contiene palabras clave. Utilizando esta información, guarda el número de esa línea para identificar el punto exacto donde comienza la sección de productos en cada ticket, lo cual es crucial para poder separar correctamente los productos del resto de la información del ticket.
...
3. El código une la información de la cabecera con todas las líneas del ticket, lo que permite asociar correctamente cada línea de producto con su respectiva cabecera. Luego, marca como "es_producto = TRUE" aquellas líneas que aparecen después de la cabecera, asegurándose de que no contienen palabras clave que indican el final del ticket. Esto ayuda a identificar y separar correctamente las líneas que corresponden a productos de las que no lo son.
...
4. Con este código, nos quedamos solo con las líneas que contienen productos realmente comprados.
...
5. Este código, cuenta cuántos productos hay en cada ticket (archivo).
...
6. Este código une la cantidad de productos por cada ticket con la fecha correspondiente, utilizando la clave común "archivo", que identifica el ticket. Luego, agrega una columna con el día de la semana (en minúsculas), que extrae el nombre del día a partir de la fecha, transformándolo en una cadena de texto en formato de día de la semana. Esto permite identificar qué día de la semana corresponde cada ticket.
...
7. Este código agrupa los tickets por el día de la semana, utilizando la columna "dia_semana", que contiene el nombre del día correspondiente a cada fecha. Luego, calcula la media de productos por ticket para cada día de la semana. Finalmente, organiza los días de la semana en su orden natural para asegurar que los días se muestren de manera secuencial, y no alfabéticamente.
...
1.  Tablas:
    ...
    -   Tabla \@ref(tab:tabla-TiposIVA): Dinero que ha recaudado el IVA del 5%, 10% y 21%.
2.  Figuras:
