...
```{r figura-bananas_platanos, fig.cap="Evolución del precio por kg del platano y la banana"}
...
```{r figura-semanas, fig.cap="Tickets recogidos de cada dia de la semana"}
...
Como se observa en la figura, si se tuviese que cerrar un día entre semana seria el jueves.
...
Finalmente, en la tabla podemos ver el resultado: la media de productos los lunes es de 17, la de los martes es de 14, la de los miércoles es de 16, la de los jueves es de 11, la de los viernes es de 16, la de los sábados es de 19 y la de los domingos es de 30. Podemos observar que, de normal, el dia que menos productos se venden es el jueves.
...
# 2. Filtrar líneas con precio €/kg y extraer precio
...
# 3. Obtener el nombre del producto desde la línea anterior
df_pesados <- df_pesados %>%
  ...
  filter(str_detect(producto, "MANZANA")) # Nos quedamos solo con manzanas

# 4. Graficar evolución del precio por kilo de las manzanas
ggplot(df_pesados, aes(x = fecha, y = precio_kg)) +
  geom_line(color = "steelblue", linewidth = 1) +
  geom_point(color = "darkred", size = 2) +
  labs(
    title = "Evolución del precio por kilo: MANZANA",
    x = "Fecha",
    y = "€/kg"
  ) +
  theme_minimal()
...
9.  ¿Cuáles son los productos menos adquiridos en el conjunto de tickets disponibles?
...
Los productos más adquiridos son queso lonchas cabra y brotes tiernos maxi y los menos adquiridos son aceite de girasol y aliñada picadedos.
...
Como podemos observar en la tabla, el producto más comprado por unidades en una sola transacción es la cola sin cafeina (8 unidades).
...
Como se puede comprobar en la gráfica se compran más platanos en los meses de enero, febrero y marzo (es decir, en invierno), por tanto, los plátanos tienen estacionalidad.
