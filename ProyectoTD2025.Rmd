...
```
```{r figura-MetodoPago}
#Realizamos un histograma para mostrar así la distribución de los importes según el método de pago
ggplot(resumen_tickets %>% filter(Metodo_Pago %in% c("Tarjeta", "Efectivo")), 
       aes(x = Importe_Total, fill = Metodo_Pago)) +
  geom_histogram(binwidth = 5, alpha = 0.6, position = "identity", color = "white") +
  labs(
    title = "Distribución del importe total por método de pago",
    x = "Importe total del ticket (€)",
    y = "Cantidad de tickets",
    fill = "Método de pago"
  ) +
  scale_fill_manual(values = c("Efectivo" = "orange", "Tarjeta" = "lightblue")) +
  theme_minimal()
```
...
1.  Tablas:
    ...
    -   Tabla \@ref(tab:tabla-ImporteTotal): Muestra el dinero total pagado con tarjeta
    -   Tabla \@ref(tab:tabla-TiposIVA): Muestra el dinero total recogido según el tipo de IVA.
...
2.  Figuras:
    ...
    -   Figura \ref(fig:figura-MetodoPago): Distribución del importe de los tickets según el método de pago.
